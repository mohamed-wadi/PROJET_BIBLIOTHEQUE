function addToWishlist(isbn) {
    ...
    xhr.open("POST", "header/AddToWishlist.php", true);
...
function loadWishlistCount() {
    var xhr = new XMLHttpRequest();
    xhr.open("GET", "header/AddToWishlist.php", true);
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            var data = JSON.parse(xhr.responseText);
            displayWishlistCount(data);
        }
    };
    xhr.send();
}

function displayWishlistCount(data) {
    var wishlistCount = document.getElementById("countwishlist");
    wishlistCount.textContent = data.sql.NLR;
}

// Appeler la fonction pour charger le nombre total dans la liste de souhaits
loadWishlistCount();
